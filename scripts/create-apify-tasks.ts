 ...
 *
 * Each task is on a single line so a line-by-line scan is safe and exact.
 */
...
async function main() {
  ...
  // Only process Indeed tasks that have no taskId yet (idempotent)
  const pending = APIFY_TASKS.filter(t => t.source === 'indeed' && t.taskId === '')

  if (pending.length === 0) {
    console.log('Nothing to do — all Indeed tasks already have taskIds.')
    return
  }

  console.log(`Found ${pending.length} Indeed tasks to create.\n`)
